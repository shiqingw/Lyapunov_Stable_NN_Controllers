...
def linearize_pendulum(system: pendulum.PendulumDynamics):
    """Linearize theta_ddot = f(theta, theta_dot, u) about the upright origin.

    PendulumDynamics.linearized_dynamics uses sin(theta) for d(theta_ddot)/dtheta,
    which is 0 at theta = 0 and therefore drops the (g/l) gravity term. We instead
    differentiate forward() with autograd, which gives the correct (g/l)*cos(0).
    """
    x = torch.zeros((1, 2), requires_grad=True)
    u = torch.zeros((1, 1), requires_grad=True)
    qddot = system.forward(x, u)
    A = torch.zeros((2, 2))
    B = torch.zeros((2, 1))
    # Position row: d(theta)/dt = theta_dot.
    A[0, 1] = 1.0
    # Velocity row: gradients of theta_ddot wrt (x, u).
    A[1], B[1] = torch.autograd.grad(qddot[0, 0], [x, u])
    return A, B


def compute_lqr(system: pendulum.PendulumDynamics):
    A, B = linearize_pendulum(system)
    A_np, B_np = A.detach().numpy(), B.detach().numpy()
    # Cost int (||x||^2 + u^2) dt  ->  Q = I, R = I.
    Q = np.eye(2)
    R = np.eye(1)
...
def approximate_lqr(
    system: pendulum.PendulumDynamics,
    ...
    def approximate(net, system_input, target, lr, max_iter):
        optimizer = torch.optim.Adam(net.parameters(), lr=lr)
        for i in range(max_iter):
            optimizer.zero_grad()
            output = torch.nn.MSELoss()(net.forward(system_input), target)
...
@hydra.main(config_path="./config", config_name="invpend_state_training.yaml")
def main(cfg: DictConfig):
    ...
    dt = cfg.model.dt
    system_continuous = pendulum.PendulumDynamics(
        m=cfg.model.m, l=cfg.model.l, beta=cfg.model.beta
    )
